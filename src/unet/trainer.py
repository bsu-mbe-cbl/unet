...
from autosegment.unet import schedulers, utils
from autosegment.unet.callbacks import (
    TensorBoardImageSummary,
    TensorBoardWithLearningRate,
)
from autosegment.unet.schedulers import SchedulerType
...
class Trainer:
    ...
    Fits a given model to a datasets and configres learning rate schedulers and
        various callbacks
    :param name: Name of the model, used to build the target log directory if
    ...
    :param tensorboard_images_callback: Flag if intermediate predictions should
        be stored in Tensorboard. Alternatively a callback instance can be
        passed
    ...
    :param scheduler_opts: Further kwargs passed to the learning rate
        scheduler.
    """
    ...
    def __init__(
        ...
        tensorboard_images_callback: Optional[
            Union[TensorBoardImageSummary, bool]
        ] = True,
    ...
    def fit(
        ...
        skip_callbacks: Optional[bool] = False,
        epochs=10,
        batch_size=1,
        **fit_kwargs,
    ):
        ...
        :param fit_kwargs: Further kwargs passed to `model.fit`
        """
        ...
        if not skip_callbacks:
            callbacks = self._build_callbacks(train_dataset, validation_dataset)

            if learning_rate_scheduler:
                callbacks += [learning_rate_scheduler]

        train_dataset = train_dataset.map(
            utils.crop_labels_to_shape(prediction_shape)
        ).batch(batch_size)
    ...
    def _get_output_shape(self, model: Model, train_dataset: tf.data.Dataset):
        return model.predict(train_dataset.take(count=1).batch(batch_size=1)).shape
    ...
    def _build_callbacks(
        ...
        if isinstance(self.checkpoint_callback, Callback):
            ...
            callbacks.append(ModelCheckpoint(self.log_dir_path, save_best_only=True))
        ...
        if isinstance(self.tensorboard_images_callback, Callback):
            ...
            tensorboard_image_summary = TensorBoardImageSummary(
                "train", self.log_dir_path, dataset=train_dataset, max_outputs=max_n_img
            )
    ...
    def _build_learning_rate_scheduler(
        self, train_dataset: tf.data.Dataset, **scheduler_opts
    ) -> Optional[Callback]:
    ...
    def evaluate(
        ...
        **kwargs,
        ...
        if test_dataset:
            shape = shape or self._get_output_shape(model, test_dataset)[1:]
            return model.evaluate(
                test_dataset.map(utils.crop_labels_to_shape(shape)).batch(batch_size=1),
                **kwargs,
            )
