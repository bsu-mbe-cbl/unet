...
class LearningRateScheduler(tf.keras.callbacks.Callback):
    ...
    def __init__(
        self, schedule: Callable[[int], float], steps_per_epoch: int, verbose=0
    ):
    ...
    def on_train_batch_begin(self, batch, logs=None):
        ...
        if self.verbose > 0:
            logger.info(
                "\nBatch %05d: LearningRateScheduler changing learning rate to %s.",
                batch + 1,
                new_lr,
            )
...
class WarmupLinearDecaySchedule:
    ...
    def __call__(self, step):
        if step < self.warmup_steps:
            learning_rate = (
                self.initial_learning_rate * float(step) / max(1.0, self.warmup_steps)
            )
        else:
            decay_factor = max(0, (self.total_steps - step) / self.decay_steps)
            learning_rate = (
                self.min_lr + (self.initial_learning_rate - self.min_lr) * decay_factor
            )
